...
describe('compat', () => {
  ...
  it('should generate inline styles', () => {
    ...
    const view = wrapper.find('div').first()
  ...
  describe('extending', () => {
    it('should combine styles of extending components', () => {
      const Parent = styled.div`opacity: 0.9;`
      const Child = Parent.extend`padding: 10px;`

      const parent = shallow(<Parent />)
      const child = shallow(<Child />)

      expect(parent.find('div').prop('style')).toEqual([
        { opacity: 0.9 },
        undefined
      ])

      expect(child.find('div').prop('style')).toEqual([
        {
          opacity: 0.9,
          paddingTop: 10,
          paddingRight: 10,
    ...
    it('should combine styles of extending components in >= 3 inheritances', () => {
      const GrandGrandParent = styled.div`background-color: red;`
      ...
      expect(grandGrandParent.find('div').prop('style')).toEqual([
        {
          backgroundColor: 'red',
        },
        undefined,
      ...
      expect(grandParent.find('div').prop('style')).toEqual([
        {
          backgroundColor: 'red',
          borderWidth: 10,
        },
  ...
  describe('expanded API', () => {
    it('should attach a displayName', () => {
      ...
      const CompTwo = styled.div.withConfig({ displayName: 'Test' })``
