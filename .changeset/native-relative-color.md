...
React Native: relative color syntax is supported. You can write values like `oklch(from #f00 calc(l - 0.15) c h)` to derive a new color from a base color, and styled-components renders the result consistently on iOS, Android, and the web. The `oklch()`, `oklab()`, `lch()`, and `lab()` forms are supported, alongside `rgb()`, `hsl()`, `hwb()`, and `color()`.
