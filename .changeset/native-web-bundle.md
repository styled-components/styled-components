...
The browser handles a handful of CSS features more accurately than any render-time approximation, so the new build defers to it:
...
The bundle is also ~10 kB smaller (gzipped: ~4 kB) than the iOS / Android bundle because the features the browser already handles are excluded.
