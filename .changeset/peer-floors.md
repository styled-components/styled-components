...
- The `enableVendorPrefixes` prop on `<StyleSheetManager>` is gone. Runtime vendor prefixing is opt-in via `prefixPlugin` from `styled-components/plugins`, covering CSS that still needs prefixes at the React JS API browser floor. Build-time PostCSS remains fine for custom targets.
