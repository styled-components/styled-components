...
describe('flatten', () => {
  ...
  it('handles nested objects', () => {
    ...
    const css = ['font-size: 14px;', '@media screen and (min-width: 250px) {', 'font-size: 16px;', '}', '&:hover {', 'font-weight: bold;', '}'];
    // $FlowFixMe
    expect(flatten([obj])).toEqual(css);
    // $FlowFixMe
    expect(flatten(['some:thing;', obj, 'something: else;'])).toEqual(['some:thing;', ...css, 'something: else;']);
  ...
  it('throws if trying to interpolate a normal React component', () => {
    ...
    expect(console.error.mock.calls[0][0]).toMatchInlineSnapshot(`"Foo is not a styled component and cannot be referred to via component selector. See https://www.styled-components.com/docs/advanced#referring-to-other-components for more details."`);
  });
  ...
  it('does not error for regular functions', () => {
    ...
    expect(() => TestRenderer.create(<SvgIcon viewBox="0 0 512 512">
          <path d="M39.6,95.6z" />
        </SvgIcon>)).not.toThrowError();
