...
describe('parseMediaQuery', () => {
  ...
  it('caps cache growth so unbounded unique inputs cannot leak memory', () => {
    // Long-lived sessions that interpolate runtime variation into a query
    // string would otherwise grow this cache without bound. The eviction
    // discipline matches the parser's other per-string caches.
    const original = parseMediaQuery('(min-width: 1px)');
    for (let i = 2; i <= 250; i++) {
      parseMediaQuery(`(min-width: ${i}px)`);
    }
    const reparsed = parseMediaQuery('(min-width: 1px)');
    expect(reparsed).not.toBe(original);
    expect(parseMediaQuery('(min-width: 1px)')).toBe(reparsed);
  });
});
