...
- Known gap, characterized in the native contract suite: a native `style` accepts web-only CSS (`float`, `'3em'`, custom properties). `TargetProps` takes no runtime parameter, so the CSS-variable widening cannot be gated on web vs native where it is applied. Verified identical on 6.4.2, 6.4.4 and current, so it is inherited rather than introduced. Closing it means threading `R` through `TargetProps` and its four call sites, which is a signature change to measure, not a patch.
