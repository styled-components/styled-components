...
- NEVER skip evaluating interpolations or attr functions across renders, and never reintroduce a props-equal render cache that does. They are the component's own render body and may call hooks or read state outside props and theme, so skipping them on a "props unchanged" render breaks the rules of hooks (a hook count that changes between renders, "Rendered fewer hooks than expected") and serves stale styles (#5788). Cross-render bailout on equal props is the wrapping `React.memo`'s job, which soundly skips the whole component; only the caller knows the full input set the interpolations depend on. A hook may still be branched on a build or runtime constant (`__SERVER__`, `IS_RSC`), which is stable across a component's renders, never on per-render data. A `use`-prefixed helper that calls no hooks is the trap that makes a data-dependent hook call look safe; don't introduce one. Two gates: `correctness/useHookAtTopLevel` in Biome, and `memoization.test.tsx`, which records the hook sequence across renders (including a hook called inside an interpolation) and asserts it stays identical.
...
- The web and native render paths mirror each other: `models/StyledComponent.ts` with `models/WebStyle.ts` for web, `models/StyledNativeComponent.ts` with `models/NativeStyle.ts` for native. When changing one, check whether the other needs the same change. A render-path, interpolation, or attr-evaluation fix on web almost always applies to native, and the reverse. Land both together or state why one is exempt.
