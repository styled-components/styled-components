...
- Links to the site use the bare domain (`https://styled-components.com`). There is no `www` subdomain -- check docs, READMEs, and runtime error strings (`base.ts`, `ComponentStyle.ts`, `flatten.ts`, `src/utils/errors.md`).
