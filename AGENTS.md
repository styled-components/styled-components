...
- Wired through `createCompiler` in `src/utils/cssCompile.ts`. The `SC_USE_STYLIS` env escape hatch was removed with stylis itself.
- Output is byte-identical to stylis output from v6: SSR class-name hash stability across the v6→v7 upgrade depends on this. Parity corpus at `src/parser/parity.test.ts` covers 60+ cases; `src/utils/test/cssCompile.test.ts` (101 cases) also runs through the in-house path via `createCompiler`.
...
- AST-direct emit (`src/parser/compile.ts`) skips per-render `preprocessCSS + parse`: walks the construction-time AST, fills sentinels with evaluated interpolation values, runs `emit-web` on the filled AST. Output is byte-equal to the string-input path because both funnel through the same parser+emitter. The `Compiler` interface exposes two named methods set up by `createCompiler`: `compile(css, selector?, prefix?, componentId?)` for string-input cases (Keyframes registration only) and `emit(source, filled, parentSelector, componentId, fragments?)` for the Source-based path that all WebStyle / NativeStyle / GlobalStyle generation uses. The callable form `compiler(css, ...)` was retired in favor of these named methods so the type system makes the input shape explicit.
- Source-everywhere invariant: every WebStyle / NativeStyle / GlobalStyle has a parseable Source. The `css\`...\`` constructor attaches one for tagged-template inputs; object input (`css({...})`) goes through `objectToTemplate` to synthesize a template; function input (`css(p => ...)`) becomes a single block-level slot; non-`css()` synthetic RuleSets get `synthesizeSourceForRuleSet(rules)` called on them at constructor time. `styled(Base)\`...\`` extension uses `concatSourceInputs` to merge the base's and extension's template inputs into a single combined Source. Result: `flatten.ts` + `joinStringArray` are deleted; the dual-path `WebStyle.generate` slow path is gone; the Compiler emit path runs once per render through the AST.
...
- `substitute` bails when a substituted value contains structural chars (`;` `{` `}`); the legacy `preprocessCSS` + parser sanitize unbalanced braces / drop malformed decls gracefully and reproducing that is out of scope for the fast path.
...
- `packages/styled-components/src/models/nativeStyleCompiler.ts` walks the parser AST once per unique CSS string and produces `{ base, conditional, keyframes, startingStyle?, resolvers? }`. Results cached as `Map<rawCSS, NativeStyles>` with FIFO eviction at `LIMIT=200` (empirically validated; see `src/native/transform/test/stress.test.ts`). Cache key is the raw input string (V8 caches a string's hash on first access; warm hits are O(1) Map.get); no manual djb2/base52 hashing.
