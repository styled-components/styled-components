...
- `OverrideStyle` applies once per render target inside `TargetProps`, never to a merged prop bag at a JSX call site. A target's props are shared by every component built on it, so the widening resolves once per tag rather than once per component; the call-site form was the largest single consumer cost. Four things are load-bearing about the current shape, each found by measurement:
