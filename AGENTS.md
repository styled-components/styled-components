...
- `__DEV__` — build-time constant gating dev-only code (warnings, validation, stack-walking diagnostics). Substituted at our rollup build: literal `true` in the standalone dev bundle, literal `false` in the standalone prod bundle, and the runtime expression `process.env.NODE_ENV !== 'production'` in every other published bundle (cjs / esm / native / native-web / plugins / reanimated). That keeps consumer-side DCE intact: webpack / vite / esbuild / Metro all substitute NODE_ENV in production builds and terser drops the dead branch. The native-showcase babel plugin substitutes `__DEV__` based on its own NODE_ENV so Metro can compile source verbatim. Inside jest the global defaults to `true`; flip per-test with `(global as ...).__DEV__ = false` to exercise the production branch. Always put `__DEV__` FIRST in compound conditions (`if (__DEV__ && something)`) so terser sees `if (false) {...}` immediately and drops the entire block. The `warnOnce` body and the native dev wrapper both have `if (!__DEV__) return;` at the top as a backstop, but callsites should still wrap so the message string and any constructed arguments tree-shake out entirely.
