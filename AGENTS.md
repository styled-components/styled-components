...
- `fillAst` / `fillNode` preserve identity for static subtrees: nodes whose subtree depends on no interpolation slot return by reference, letting `fillAst` skip allocating a new `out` array when nothing diverged. The `interpolated(node)` predicate (lazily computed, WeakMap-cached on Node identity) is the gate; Rule and AtRule cases additionally lazy-clone selectors / preserve children identity per-section, so a Rule with static selectors but dynamic children reuses its selectors array. For a typical mixed component (1 dynamic decl among 14 static parts) this drops per-render allocations to only the dynamic Decls.
...
- Runtime sentinel resolution (createTheme tokens, `env()`, viewport / container units, `light-dark()`, nested CSS fns) follows a single contract: every place a CSS custom property could legitimately interpolate, our resolver pipeline does. Property values flow through `buildResolver` in `polyfills/resolvers.ts` which dispatches by prefix to math / color / template / theme / env / light-dark resolvers; pass-through props with embedded sentinels assemble at render time via `templateResolver`. `@media` / `@container` feature values unwrap to their createTheme fallback at `parseMediaQuery` time (the parsed Query is cached by raw string). `@starting-style` and `@keyframes` frame decls run through `extractResolvers` so theme-driven animation values resolve at render time. Composite-string theme leaves round-trip through `escapeSentinelFallback` / `unescapeSentinelFallback`. The `warnIfSentinelLeak` guard in `transform/dev.ts` catches `${p => p.x + t.space.xl}`-style JS-arithmetic concat. `project_native_sentinel_gaps.md` (memory) tracks the matrix.
